...
- **`src/schema/`**: a Pydantic v2 domain model covering documents, drugs, development programs, trials, regulatory events, market metrics, and provenance.
...
A defining design choice runs through all of it: the language model is given only the calls that need judgment. Everything testable stays in code: the loop, the stopping rule, the decision to escalate, the link from each claim to its evidence. The reasoning behind this and the other major decisions is in [engineering decisions](docs/ENGINEERING_DECISIONS.md); the full system design is in [architecture](docs/ARCHITECTURE.md).
...
The **agent's results are documented, not reproducible from a clean clone**, which is a deliberate consequence of the project's constraints, not an omission. Running the agent end to end needs the private corpus and a live model key, so the headline numbers live in the case study as findings rather than behind a one-command rerun. The question the project actually cares about, whether the measurement is trustworthy, is answerable by reading the committed scorer and goldens, without re-running the model.
